...
% Initialize variables to store permutation results
permutation_accuracy = zeros(length(selchans), num_conditions, num_permutations);

% Initialize variables to store significance results
p_values = zeros(length(selchans), num_conditions);
significant_decoding = zeros(length(selchans), num_conditions);

% Prepare the data for decoding as before
for ch_idx = 1:length(selchans)
    ch = selchans(ch_idx);
    for w = 1:num_windows
        window_start = (w-1) * window_size_samples + 1;
        ...
        for perm = 1:num_permutations
            shuffled_labels = flattened_labels_window(randperm(length(flattened_labels_window)));
            perm_confusion_matrix = zeros(num_conditions, num_conditions);
            ...
            for fold = 1:cv.NumTestSets
                ...
                for i = 1:length(test_labels)
                    perm_confusion_matrix(test_labels(i), predicted_labels(i)) = ...
                        perm_confusion_matrix(test_labels(i), predicted_labels(i)) + 1;
                end
            end
            
            % Calculate permutation accuracy for each condition
            for cond_idx = 1:num_conditions
                permutation_accuracy(ch_idx, cond_idx, perm) = ...
                    perm_confusion_matrix(cond_idx, cond_idx) / sum(perm_confusion_matrix(cond_idx, :));
            end
        end

        % Calculate p-values based on permutation distribution
        for cond_idx = 1:num_conditions
            observed_accuracy = decoding_accuracy_conditions(ch_idx, cond_idx);
            perm_distribution = squeeze(permutation_accuracy(ch_idx, cond_idx, :));
            p_values(ch_idx, cond_idx) = mean(perm_distribution >= observed_accuracy);
            if p_values(ch_idx, cond_idx) < 0.05
                significant_decoding(ch_idx, cond_idx) = 1;
            end
        end
    end
end

% Visualization of significant decoding conditions
figure;
imagesc(1:length(selchans), 1:num_conditions, significant_decoding');
xlabel('Channels');
ylabel('Conditions');
title('Significant Decoding Conditions by Channel');
colorbar;
...
% Highlight significant best conditions
significant_best_conditions = best_conditions .* significant_decoding(:, best_conditions)';

figure;
bar(significant_best_conditions);
xlabel('Channels');
ylabel('Best Decoded Condition (if significant)');
